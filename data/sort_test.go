...
func TestUnitReviewSortSuccess(t *testing.T) {
	...
	Convey("Given an empty sort", t, func() {
		...
		Convey("When reviewSort is called", func() {
			reviewSort(ctx, urlQuery, validatedQueryParams, cfg.DefaultSort.Other)

			Convey("Then set default sort and localisation key for default to validatedQueryParams", func() {
				So(validatedQueryParams.Sort.Query, ShouldEqual, cfg.DefaultSort.Other)
				So(validatedQueryParams.Sort.LocaliseKeyName, ShouldEqual, sortOptions[cfg.DefaultSort.Other].LocaliseKeyName)
			})
		})
	})
