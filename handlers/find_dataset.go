...
func NewFindDatasetConfig(req *http.Request) AggregationConfig {
	createPageModel := func(cfg *config.Config, req *http.Request, basePage core.Page, validatedQueryParams data.SearchURLParams, categories []data.Category, topicCategories []data.Topic, searchResp *searchModels.SearchResponse, lang string, homepageResp zebedee.HomepageContent, _ string, navigationCache *models.Navigation, _ string, censusTopicCache cache.Topic, errorMessage []core.ErrorItem, _ zebedee.PageData, _ []zebedeeCli.Breadcrumb) model.SearchPage {
